const mdDemo = async(req, res) => {
    try{
        const markdown = `# Senior Software Development Engineer (SDE) - Bangalore
...
`
        res.status(200).json({
            success: true,
            markdown: markdown,
            message: "Demo JD sent"
        })

    }catch(err){
        console.error('Error sending jd:', err);
        res.status(500).json({success:false,  message: 'Failed to send jd', err  });
    }
}
